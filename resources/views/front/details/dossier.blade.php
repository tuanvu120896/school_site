                    ...
                    <div class="main-list" style="">
                        <div id="main-content">
                            <div style="padding: 10px 10px 5px 10px;"><h1
                                        style="font-size: 18px">{{substr(ucfirst($dossier['name']),0,110)}}</h1></div>
                            <p style="padding: 0 10px;">
                                <img width="12" height="12" src="{{url('asset/front/SP/image/calendar-icon.png')}}">
                                <span>{{date("F j, Y", strtotime($dossier['created_at']))}}</span>
                                <img width="12" height="12" src="{{url('asset/front/SP/image/folder-icon.png')}}">
                                <span><a href="{{url(route('front_dossier_list'))}}" style="color: #079b9b">Hướng Dẫn
                                        Làm Hồ Sơ</a></span>
                            </p>
                            <div style="text-align: center;margin: 5px 0 5px 0 ;">
                                <img src="{{url('asset/image/icon/dossier/'.$dossier['image'])}}" width="80%"
                    ...
                    <div class="category-box hoc-tieng-nhat clearfix">
                        ...
                        <div id="main-reg" style="margin-top: 5px">
                            <div style="width: 100%;height: auto;">
                                @foreach($dossier_reg as $item)
                                    <div style="width: 100%; height: 100px;float: left;margin-left: 5px;line-height: normal">
                                        <a href="{{url(route('front_dossier_detail',['name' => $item->title]))}}">
                                            <img src="{{url('asset/image/icon/dossier/'.$item->image)}}"
                                                 width="30%" height="80px"
                                                 style="float: left;border-radius: 3px; border: 1px solid #e3e3e3">
                                        </a>
                                        <a href="{{url(route('front_dossier_detail',['name' => $item->title]))}}"><p
        ...
        <div id="container">
            <div class="frame">
                <ul id="pan" class="clearfix">
                    ...
                    <li><a href="{{url(route('front_dossier_list'))}}">Hướng Dẫn Làm Hồ Sơ</a></li>
                    <li>{{substr($dossier['name'],0,75).'...'}}</li>
                </ul>
                <!-- //▼CONTENT▼// -->
                <div id="contents" class="clearfix">
                    ...
                    <div id="mainArea">
                        <div id="main-content">
                            <div style="font-size: 20px"><h1>{{substr(ucfirst($dossier['name']),0,110)}}</h1></div>
                            <p style="margin: 5px 0;">
                                <img width="12" height="12" src="{{url('asset/front/SP/image/calendar-icon.png')}}">
                                <span>{{date("F j, Y", strtotime($dossier['created_at']))}}</span>
                                <img width="12" height="12" src="{{url('asset/front/SP/image/folder-icon.png')}}">
                                <span><a href="{{url(route('front_dossier_list'))}}">Hướng Dẫn Làm Hồ Sơ</a></span>
                            </p>
                            ...
                            <div style="width: 100%;height: auto;">
                                @foreach($dossier_reg as $item)
                                    <div style="width: 46%; height: 100px;float: left; margin-left: 10px;">
                                        ...
                                        <a href="{{url(route('front_dossier_detail',['name' => $item->title]))}}"><p
                                                    style="float: left;color: #079b9b;width: 200px;height: 100px; margin-left: 5px;">{{$item->name}}</p>
                                        </a>
                                    </div>
                                @endforeach
                            </div>
